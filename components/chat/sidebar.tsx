...
function SidebarContent({ aboutOpen, setAboutOpen }: { aboutOpen: boolean; setAboutOpen: (open: boolean) => void }) {
  return (
    <>
      {/* Logo and Name */}
...
export function Sidebar({ isMobile = false }: SidebarProps) {
  ...
  // Mobile sidebar - content only, parent handles scrolling
  if (isMobile) {
    return (
      <div className="flex flex-col min-h-full w-full bg-sidebar">
        <SidebarContent aboutOpen={aboutOpen} setAboutOpen={setAboutOpen} />
      </div>
    )
  }

  // Desktop sidebar - with its own ScrollArea
  return (
    <aside className="hidden md:flex flex-col w-72 bg-sidebar border-r border-sidebar-border h-screen overflow-hidden">
      {/* Logo and Name */}
      <div className="p-4 flex items-center gap-3">
        <div className="rounded-lg bg-sidebar-primary/10 p-2">
          <Brain className="h-6 w-6 text-sidebar-primary" />
